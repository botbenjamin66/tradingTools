...
mode = 'mac'
cutOffDate = pd.to_datetime('01.01.2012', format='%d.%m.%Y')
trail1, trail2 = 120, 400
...
def createDfs():
    ...
    # STATS
    dfTransactions[f'avgWin{trail1}d'] = dfTransactions['totalReturnPf'].rolling(trail1, min_periods=1).apply(lambda x: np.nanmean(x[x > 0]) if np.any(x > 0) else np.nan)
    dfTransactions[f'avgLoss{trail1}d'] = dfTransactions['totalReturnPf'].rolling(trail1, min_periods=1).apply(lambda x: np.nanmean(x[x < 0]) if np.any(x < 0) else np.nan)
    dfTransactions[f'avgWin{trail2}d'] = dfTransactions['totalReturnPf'].rolling(trail2, min_periods=1).apply(lambda x: np.nanmean(x[x > 0]) if np.any(x > 0) else np.nan)
    dfTransactions[f'avgLoss{trail2}d'] = dfTransactions['totalReturnPf'].rolling(trail2, min_periods=1).apply(lambda x: np.nanmean(x[x < 0]) if np.any(x < 0) else np.nan)
    dfTransactions[f'hitRatio{trail1}d'] = dfTransactions['totalReturnPf'].rolling(trail1, min_periods=1).apply(lambda x: np.sum(x[x > 0]) / np.sum(x) if np.sum(x) != 0 else np.nan)
    dfTransactions[f'hitRatio{trail2}d'] = dfTransactions['totalReturnPf'].rolling(trail2, min_periods=1).apply(lambda x: np.sum(x[x > 0]) / np.sum(x) if np.sum(x) != 0 else np.nan)
...
def addSdax(df, cutOffDate):
    sdax_data = yf.Ticker("^SDAXI").history(start=cutOffDate)[['Close']]
    sdax_data['Close'] = sdax_data['Close'].round(1)
    sdax_data.set_index(sdax_data.index.date, inplace=True)
    df['sdaxClose'] = df.index.to_series().map(lambda x: sdax_data['Close'].get(x, None))
    df['sdaxClose'].fillna(method='ffill', inplace=True)
    return df
...
# DASHBOARD
dashboardDf = df.drop(columns=['Time', 'Beschreibung', 'ISIN', 'deltaContracts', 'Contracts', 'Price', 'Account', 'Trades', 'costBasis'])
