...
cutOffDate = pd.to_datetime('01.01.2020', format='%d.%m.%Y')
trail1, trail2 = 50, 500
...
def createDfs():
    ...
    if len(latestFiles) > 1:
        ...
        dfTransactions['Equity'].fillna(method='ffill', inplace=True)
...
def addSdax(df, cutOffDate):
    ...
    df['sdaxClose'] = df.index.map(sdax_data['Close'])
